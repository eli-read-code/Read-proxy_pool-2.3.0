...
[2],__import__() 函数用于动态加载类和函数。如果一个模块[xxx.py]经常变化就可以使用 __import__() 来动态载入。此处载入的是:redisClient.py模块(文件)

[3],getattr(__import__(__type), "%sClient" % self.db_type.title())(host=...) 动态调用了redisClient.py[或ssdbClient.py]模块中的redisClient[或ssdbClient]类,并且使用参数初始化了.
"""
__author__ = 'JHao'
...
class DbClient(withMetaclass(Singleton)):
    ...
    def __initDbClient(self):
        ...
        # eli:: __import__() 函数用于动态加载类和函数。如果一个模块[xxx.py]经常变化就可以使用 __import__() 来动态载入。此处载入的是:redisClient.py模块(文件)
        # eli::getattr(__import__(__type), "%sClient" % self.db_type.title())(host=...) 动态调用了redisClient.py[或ssdbClient.py]模块中的redisClient[或ssdbClient]类,并且使用参数初始化了.
